...
- **ui-motion-design**: The easing/timing/spring-physics *principles* — use that skill to decide what motion should communicate, this skill and its siblings (`gsap-timeline`, `gsap-scrolltrigger`, `gsap-react`, `gsap-frameworks`, `gsap-utils`, `gsap-performance`, `gsap-plugins`) for the actual GSAP implementation.
- **design-templates**: Apply GSAP-driven motion to the device-frame and product-3d-tilt templates instead of leaving them fully static.
- **claude-code-tooling**: Documents GSAP as a real animation library alongside other external tools; this vendored pack is the official Claude Code skill wrapper around it.

## Notes

Vendored for real from [greensock/gsap-skills](https://github.com/greensock/gsap-skills) (MIT, official GreenSock repo) — see `SOURCE.md` in this directory for the exact commit. One of 8 official GSAP skills vendored together (`gsap-core`, `gsap-timeline`, `gsap-scrolltrigger`, `gsap-react`, `gsap-frameworks`, `gsap-utils`, `gsap-performance`, `gsap-plugins`) from "4 Secret Claude Skills for Motion" (@hiitzroman).
