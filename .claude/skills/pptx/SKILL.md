...
- **sales-enablement**, **business-intelligence-report**, **ceo-reporting-skill**, **quarterly-okr-architect**: Common content sources when the deliverable needs to be an actual editable `.pptx` rather than an HTML deck.
- **premium-html-presentation** / **the-design-system-prompt**: Use one of those instead when a web/HTML deck (not an editable PowerPoint file) is what's wanted.
- **theme-factory**: Ready-made visual theme for the deck's palette/typography.
- **pdf**: Export the finished deck to PDF for sharing when PowerPoint editability isn't needed.
