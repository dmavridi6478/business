...
| `coreyhaines31/marketingskills` (50 growth/marketing skills bundled) | Marketing skill bundle | **Fully vendored** (Batch 82 follow-up) — all 50 skills copied into `.claude/skills/` at commit `5b2c0007766c6a1cf1d53fd8fc73e979e0821022`, MIT. The 5 that collided by name with this repo's own pre-existing skills (`content-strategy`, `copywriting`, `marketing-psychology`, `sales-enablement`, `competitor-profiling` — themselves independently derived from an earlier, different carousel, not copies of this pack) were vendored under a `-corey-haines` suffix instead of overwriting the existing ones; the other 45 kept their original names |
...
## Diagram renderer & official plugin directory ("daily briefing" batch, @githubnow / @martiendejong_dev)

Both confirmed live via GitHub search before being added:

| Repo | What it's for | Install / link |
|---|---|---|
| `tt-a1i/archify` | Agent skill that compiles a plain-English system description into an interactive, verified-topology diagram (architecture, workflow, sequence, data-flow, lifecycle) — agents produce typed JSON IR, rendering is deterministic, output is self-contained HTML with dark/light themes, motion, and before/after topology comparison. 54.5k★, JavaScript. Cross-posted by two different accounts with different star snapshots (17.4k → 54.5k) — same repo, just growing fast, not two competing projects. | https://github.com/tt-a1i/archify |
| `anthropics/claude-plugins-official` | The actual Anthropic-managed official Claude Code plugin directory — distinct from the already-documented `anthropics/claude-plugins-community` (community marketplace, 65th README addition). This is the one the `@claude-plugins-official` suffix in this repo's own `claude-code-setup-plugin` skill and `ai-second-brain`'s iMessage-channel command refers to; worth knowing it's a browsable repo in its own right, not just an install-string suffix. 36k★, Python. | https://github.com/anthropics/claude-plugins-official |

`freestylefly/awesome-gpt-image-2` (prompt-engineering template library for
GPT-Image-2, 530+ reverse-engineered examples) also appeared in this same
...
- **agent-memory-repos**: `volcengine/OpenViking` is documented in both files — its context/memory/RAG-unification angle fits that skill's table, its "installable Claude Code tool" angle fits this one. `mksglu/context-mode` was added to that skill's table instead of here, since its job (cutting MCP tool-output bloat) is a memory/context-management concern first.
- **ai-engineering-portfolio**: The `archify` entry above is a fit for that skill's "Multimodal Document AI" and general portfolio-building briefs if the user wants a diagram-generation tool to pair with one of those projects.

## Notes

Sources: a "7 Claude Code repos I can't work without" screenshot carousel (@joshualevi.ai) for the first six tools; "AI Power User Stack — 5 AI browser agents that can do the clicking for you" carousel (@zhiprompts) for the browser-automation frameworks; "Buzz — The Hive Mind for Humans + Agents" carousel (@iunlockedai, Tool Drop 15) for `block/buzz`; "AI Power User Stack — 5 open AI video models that can replace a production day" carousel (@zhiprompts) for the video-generation model repos; "22 Skills to Install in Claude" carousel (Elshad Karimov, appmillers.com) for the Build/Design/Research/Marketing collection — each entry live-checked against GitHub search before being documented, given this ecosystem's heavy same-name-fork collisions; "5 repos every Claude power user installs first" carousel (@zhiprompts) for the power-user CLI/MCP tools, also live-verified before documenting; "4 Secret Claude Skills for Motion" carousel (@hiitzroman) for the motion-skill packages — all 4 were actually cloned and read before deciding to vendor (`gsap-core` + 7 siblings) or document-only (the rest); a "Three tools you will wish you knew sooner" GitHub-trending daily briefing (@githubnow) and an "8 GitHub repos" carousel (@martiendejong_dev) for the diagram-renderer and official-plugin-directory entries — both accounts cross-posted overlapping repos (archify, awesome-gpt-image-2, free-claude-code, ai-job-search, claude-obsidian) on the same day, cross-checked against each other and against this file's prior batches before deciding what was actually new.
