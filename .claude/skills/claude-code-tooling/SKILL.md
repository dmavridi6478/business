...
| GSAP (GreenSock) | Production-grade JavaScript animation library | Real, well-known — not a "skill," a real npm library (greensock.com). Reference directly in `ui-motion-design`/`design-dev-resources` builds rather than treating it as an installable Claude skill |
