...
This skill was vendored as instructions only; the upstream repo's `/honey`
slash command and its persistence hook (`hooks/honey-state.js`) were not
vendored (see `SOURCE.md`). Without them, treat any request to switch
intensity ("honey ultra", "honey off") as scoped to the current
conversation only — say so in one line rather than claiming persistence
across sessions.
