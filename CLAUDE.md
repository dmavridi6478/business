...
```bash
/usr/local/bin/hyperresearch scholar search "<query>" --limit 25 -j          # every available source, deduplicated
/usr/local/bin/hyperresearch scholar search "<query>" --scope papers -j      # literature only, no trials/filings/series
/usr/local/bin/hyperresearch scholar search "<query>" -s openalex -s core -j # pick specific sources
/usr/local/bin/hyperresearch scholar sources -j                              # what is available, what each covers
```
...
- `/usr/local/bin/hyperresearch note show <id> -j` carries an `oa` block with `body_is_not_from_source: true`,
